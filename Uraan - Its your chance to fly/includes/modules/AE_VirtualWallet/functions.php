...
if( !function_exists('ae_credit_balance_info') ){
    ...
    function ae_credit_balance_info($user_id){
        ...
        $checkout = AE_WalletAction()->getUserWallet($user_id, 'checkout');
        $minimum = ae_get_option('minimum_withdraw', 0);
        $balance_info = array(
            'total_text'=>  mje_format_price($total->balance),
            'available_text'=>mje_format_price($available->balance),
            'freezable_text'=> mje_format_price($freezable->balance),
            'withdrew_text'=> mje_format_price($withdrew->balance),
            'working_text' => mje_format_price($working->balance),
            'checkout_text' => mje_format_price($checkout->balance),
            'total'=> $total,
            'available'=> $available,
            'freezable'=> $freezable,
            'withdrew' => $withdrew,
            ...
            'min_withdraw_text'=> mje_format_price($minimum)
        );
        return $balance_info;
    }
}
...
if(!function_exists('ae_price_format')) {
    function ae_price_format($amount, $style = '<sup>') {
        ...
        $decimal = apply_filters( 'ae_price_decimal', (isset($number_format['et_decimal'])) ? $number_format['et_decimal'] : get_theme_mod('et_decimal', 2) );
