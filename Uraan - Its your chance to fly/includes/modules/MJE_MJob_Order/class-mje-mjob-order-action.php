...
class MJE_MJob_Order_Action extends MJE_Post_Action{
    ...
    public function after_process_payment( $payment_return, $data ){
        ...
        if( $order_data['product_type'] == 'mjob_order' ) {
             ...
             */
            if( $order_data['payment'] != 'cash' && strtoupper( $payment_return['payment_status'] ) == 'COMPLETED' ) {
            ...
            else if( strtoupper( $payment_return['payment_status'] ) == 'DRAFT' )
            {
    ...
    public function resolve_dispute( $request ) {
        ...
        if( $order->post_status == 'disputing' ) {
            if(is_super_admin($user_ID)) {
                ...
                // Update balance
                $this->update_winner_revenue( $request['winner'], $order );

				if(isset($request['check_for_refun_fee_check'][0])){
					$buyer=get_post_meta($order_id,'buyer_id',true);
					if($buyer==$request['winner']){

						// refund;
						$total_x=get_post_meta($order_id,'amount',true);
						$fee_x=get_post_meta($order_id,'fee_commission',true);
     ...
     * @author JACK BUI
     */
    public function update_winner_revenue( $winner, $order ) {
        if ( $order ) {
                ...
                if ( !$is_transferred ) {
                    if ( $winner == $order->seller_id ) { // Seller win
                        ...
                        if( $order->payment_type == 'credit' ) {
                            ...
                            $buyer_wallet_spent->balance -= $order->real_amount; // amount with commission
