...
function filter_query_advance($query_args)
{
    ...
    if(isset($query['language']) && !empty($query['language'])) {
        ...
        $sql = "SELECT DISTINCT post_author FROM `wp_posts` as wp JOIN wp_term_relationships AS wtr ON wp.ID = wtr.object_id WHERE post_type = 'mjob_profile' AND wp.post_status = 'publish' AND wtr.term_taxonomy_id IN ($language)";
