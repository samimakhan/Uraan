...
class AE_Language extends AE_Base
{
    ...
    function add_new_lang() {
        ...
        if ($lang_name == "" || in_array($lang_name, $lang_arr)) {
            $return = array(
                'success' => false,
                'msg' => __("Invalid file name!", 'enginethemes')
            );
        } else {
            if ($this->generate_mo($lang_name)) {
                ...
                $return = array(
                    'success' => true,
                    'msg' => __("Adding new language successfully.", 'enginethemes') ,
                    'data' => array(
                        'lang_name' => $lang_name,
                        'ID' => $lang_name
                    )
    ...
    ?>
        ...
        <div id="notice_wizard" class="et-create-folder-notice">
            <p>
            <?php
                printf( __("Theme functions required to create a folder wp-content/et-content. Please set chmod folder wp-content to 755.", 'enginethemes')) ;
            ?>
            </p>
        </div>
    <?php
    ...
    public static function load_text_domain() {
        ...
        if (in_array($selected_lang, get_available_languages(THEME_LANGUAGE_PATH))) load_textdomain('enginethemes', THEME_LANGUAGE_PATH . "/$selected_lang.mo");
        else load_textdomain('enginethemes', DEFAULT_LANGUAGE_PATH . "/$selected_lang.mo");
    }
    ...
    function save_language() {
        ...
        if (empty($singular) || empty($translation) || empty($context)) {
            wp_send_json(array(
                'success' => true,
                'msg' => __("There was no changes in your translation.", 'enginethemes')
            ));
        }
        ...
        wp_send_json(array(
            'success' => true,
            'msg' => __("Translation saved! ", 'enginethemes')
        ));
    }
    ...
    function load_translation_form() {
        ...
        */
        ...
        foreach ($trans_arr as $key => $value) {
            ...
            $data.= '<div class="form-item">
					...
					<textarea type="text"  name="translations[]" class="autosize" row="1" style="height: auto;overflow: visible;"
						placeholder="' . __("Type the translation in your language", 'enginethemes') . '" >' . $translate_txt . '</textarea>
				</div>';
        ...
        $return = array(
            'success' => true,
            'data' => $data,
            'msg' => __("Loading successfully!", 'enginethemes') ,
            'tran_arr' => $trans_arr
        );
