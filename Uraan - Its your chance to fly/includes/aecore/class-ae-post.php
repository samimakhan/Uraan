...
class AE_PostAction extends AE_Base
{
    ...
    function fetch_post() {
        ...
        ob_start();
        ae_pagination($data['query'], $page, $_REQUEST['paginate'], $load_more_text);
        $paginate = ob_get_clean();
        $data = apply_filters('response_fetch_'.$this->post_type , $data, $query_args);
        /**
         * send data to client
         */
        if (!empty($data)) {
