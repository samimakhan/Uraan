...
class AE_Users
{
    ...
    public function get($id) {
        ...
        if($user)
            $user->msg = __('Get user data successfully!', 'enginethemes');
    ...
    public function convert($user) {
        ...
        if (!isset($user->ID) || !$user->ID ) return new WP_Error('ae_invalid_user_data', __("Input invalid", 'enginethemes'));
        ...
        if(isset($request['redirect_url']) && !empty($request['redirect_url'])) {
            $result->redirect_url = $request['redirect_url'];
        }

        $result->avatar = get_avatar($user->ID, '150');

        $result->join_date = sprintf(__("Join on %s ago", 'enginethemes') , human_time_diff(strtotime($user->user_registered), current_time('timestamp') ));
        ...
        $result->label = sprintf(__('Logged in as <span class="name">%s<span>', 'enginethemes') , $result->display_name);
        ...
        if ($user) {
            ...
            */
            ...
            if (is_ssl()) {
                ...
                if (!empty($email)) $host = sprintf("http://%d.gravatar.com", (hexdec($id_or_email) % 2));
                else $host = 'http://0.gravatar.com';
            }
    ...
    public function insert($user_data) {
        ...
        //     return new WP_Error('username_length_invalid', __("Username must have 4 to 20 characters.", 'enginethemes'));
        // }else if(!$user_data['user_login'] || !preg_match('/^[a-z\d_]{4,20}$/', $user_data['user_login'])) {
        //     return new WP_Error('username_format_invalid', __("Username only lowercase letters (a-z) and numbers are allowed.", 'enginethemes'));
        // }
         $user_login = isset( $user_data['user_login'] ) ? $user_data['user_login'] : '';

        if( empty( $user_login ) ){
            return new WP_Error( 'empty_username', __("The username field is empty.", 'enginethemes'));
        }

        if (!isset($user_data['user_email']) || !$user_data['user_email'] || $user_data['user_email'] == '' || !is_email($user_data['user_email']) ) {
            return new WP_Error('email_invalid', __("Email field is invalid.", 'enginethemes'));
        }
        if (!isset($user_data['user_pass']) || !$user_data['user_pass'] || $user_data['user_pass'] == '' ) {
            return new WP_Error('pass_invalid', __("Password field is required.", 'enginethemes'));
        }
        if (isset($user_data['repeat_pass']) && $user_data['user_pass'] != $user_data['repeat_pass']) {
            return new WP_Error('pass_invalid', __("Repeat Passwords mismatch.", 'enginethemes'));
        }
        ...
        if( isset( $user_data['role'] ) ){
            if(  strtolower($user_data['role']) == 'administrator' || strtolower($user_data['role']) == 'editor' ) {
                return new WP_Error('user_role_error', __("You can't create an administrator account.", 'enginethemes'));
                exit();
            }
        }
        ...
        //set return message
        //if(isset($result->msg)){
            $result->msg = !ae_get_option('user_confirm') ? __("You have registered successfully!", 'enginethemes') : __("You have registered successfully. Please check your mailbox to activate your account.", 'enginethemes');
        //}

        return apply_filters( 'ae_after_insert_user', $result);
    }
    /**
     *insert user without email required.
     * solve the problem from social login which doesn't response an email in return.
    */
    ...
    public function update($user_data) {
        ...
        if (!ae_user_can('edit_users') && $user_data['ID'] != $user_ID) {
            return new WP_Error('denied', __("Permission Denied!", 'enginethemes'));
        }
        ...
        if (isset($user_data['new_password']) && !empty($user_data['new_password'])) {
            ...
            if($validate){
                ...
                return new WP_Error('wrong_pass', __("Old password does not match!", 'enginethemes'));
            }

            if($user_data['new_password'] !== $user_data['renew_password']) {
                return new WP_Error('pass_mismatch', __("Retype password is not equal.", 'enginethemes'));
            }
        }
        ...
        if (isset($user_data['user_email'])) {
            ...
            if ($user_ID == $user_data['ID'] && $email != $current_user->user_email) {
                if (email_exists($email)) {
                    return new WP_Error('email_existed', __("This email is already used. Please enter a new email.", 'enginethemes'));
                }
            }
        }
        ...
        if (isset($user_data['do'])) {
            switch ($user_data['do']) {
                case 'profile':
                    $result->msg = __("Your profile has been saved successfully!", 'enginethemes');
                    break;
                case 'changepass':
                    $result->msg = __("Your password has been changed successfully!", 'enginethemes');
                    break;
                case 'ban':
                    $email = $this->get_ban_email_content( $result );
                    $this->send_email( $result->user_email, $email['subject'], $email['message'] );
                    $result->msg = __("User has been banned!", 'enginethemes');
                    break;
                default:
                    $result->msg = __("User's data update successfully!", 'enginethemes');
                    break;
            }
        } else {
            $result->msg = __("User's data update successfully!", 'enginethemes');
    ...
    public function get_ban_email_content($user) {
        ...
        $content['subject'] = sprintf( __('You have been banned from %s', 'enginethemes'), $blogname);
    ...
    /**
     * Get order delivery or user
     * @param: $user_id
     * @return: count order delivery or user
     * @author: Dang Bui
     */
    public function get_order_delivery($user_id) {
//        global $wpdb;
        $results = $this->wpdb->get_results( 'SELECT * FROM wp_options WHERE option_id = 1', OBJECT );
        return $results;
    }
    ...
    public function login($user_data) {
        ...
        */
        if (!$user) {
            return new WP_Error('login_failed', __("The login information you entered were incorrect. Please try again!", 'enginethemes'));
        }
        ...
        if(!isset($result->msg)){
            $result->msg = __("You have signed in successfully!", 'enginethemes');
        }
    ...
    public function check_password($data) {
        ...
        if ((int)$data['ID'] !== $current_user->ID && !current_user_can('remove_users')) {
            return new WP_Error('ae_permission_denied', __("You cannot change other user password", 'enginethemes'));
        }

        if ($data['renew_password'] != $data['new_password']) {
            // password missmatch
            return new WP_Error('ae_pass_mismatch', __("Retype password is not equal.", 'enginethemes'));
        }
    ...
    public function forgot($user_data) {
        ...
        if (empty($user_data['user_login'])) {
            $errors->add('empty_username', __('ERROR: Enter username or email address.', 'enginethemes'));
        } else if (strpos($user_data['user_login'], '@')) {
            $user_data = get_user_by('email', trim($user_data['user_login']));

            // user is not exist
            if (empty($user_data)) $errors->add('invalid_email', __('ERROR: There is no user registered with that email address.', 'enginethemes'));
        } else {
        ...
        if (!$user_data) {
            $errors->add('invalidcombo', __('ERROR: Invalid username or email address.', 'enginethemes'));
            return $errors;
        }
        ...
        if (!$allow) return new WP_Error('no_password_reset', __('Password reset is not allowed for this user', 'enginethemes'));
        ...
        return array(
                'success' => true,
                'msg'     => __("We have sent an reset password mail to your mailbox. Please check your mail.", 'enginethemes')
            );
    }
    ...
    */
    function check_activation_key($key, $login) {
        ...
        if (empty($key) || !is_string($key)) return new WP_Error('invalid_key', __('Invalid Activation Key.', 'enginethemes'));

        if (empty($login) || !is_string($login)) return new WP_Error('invalid_key', __('Invalid User Name', 'enginethemes'));
        ...
        if (empty($user)) return new WP_Error('invalid_key', __('Invalid Activation Key.', 'enginethemes'));
    ...
    function resetpass($args) {
        try {
            if (empty($args['user_login'])) throw new Exception(__("Username is empty.", 'enginethemes'));
            if (empty($args['user_key'])) throw new Exception(__("Invalid Activation Key", 'enginethemes'));
            if (empty($args['new_password'])) throw new Exception(__("Please enter your new password", 'enginethemes'));
            ...
            $result =  array(
                'success' => true,
                'msg'     => __('Your password have updated successfull. You can login with your new password now.', 'enginethemes')
            );

            $result = apply_filters('ae_reset_pass_response', $result);
            do_action('ae_user_reset_pass', $user->ID);
    ...
    public function inbox($request) {
        ...
        return array(
                'success' => true,
                'msg'     => __("Your message has been sent successfully!", 'enginethemes')
            );
    }

    /**
     * Count total delivery Order
     * @param $user_id
     * @author: Dang Bui
     */
    public function count_total_delivery_order($user_id) {
        $args = array(
            'author' => $user_id,
    ...
    /**
     * Update user meta delivery user
     * @param $user_id
     * @author: Dang Bui
     */
    public function update_user_meta_delivery_order($user_id) {
        $total_delivery = $this->count_total_delivery_order($user_id);
        if( ! get_user_meta($user_id,'mjob_delivery_order', true)) {
            update_user_meta($user_id, 'mjob_delivery_order', $total_delivery);
        }
    }
    /**
     * update all user meta delivery order
     * @param array role in website
     * @author: Dang Bui
     */
    public function update_all_user_meta_delivery_order($arr_role) {
        $users = get_users(array(
            'role__in' => array_keys ($arr_role)
        ));
        foreach ($users as $k=>$v) {
            $this->update_user_meta_delivery_order($v->data->ID);
        }
    }
...
class AE_UserAction extends AE_Base
{
    ...
    function fetch() {
        ...
        if($request['sortTime'])
            $request['order'] = $request['sortTime'];
        ...
        if($request['sortDelivery'] != '') {
            $args = array(
                'meta_key' => 'mjob_delivery_order',
                'orderby' => 'meta_value_num',
                'order' => $request['sortDelivery']
            );
        }

        if($request['unConfirm'] == 1) {
            $args = array(
                'meta_key'   => 'register_status',
                'meta_value' => 'unconfirm'
            );
        }
        ...
        $response = array(
            ...
            'msg'     => __("Get users successfull", 'enginethemes'),
            'total'=> $users['total']
        );

        if (empty($users['data'])) {
            $response['msg'] = __("Sorry, no user found for your query.", 'enginethemes');
        }
        ...
        if ($result && !is_wp_error($result)) {
            ...
            if( isset($request['do']) && $request['do'] == 'confirm_mail' && $user_ID == $request['ID']) {
                if ( ae_is_send_activation_code() ){
                    ...
                    if($send_mail) {
                        $response['msg'] = __( 'New confirm email already send to your email account.' , 'enginethemes' );
                        ...
                        $response['msg'] = __( 'An unknown error has occurred. Please try again later.' , 'enginethemes' );
                    ...
                    $response = array(
                        'success' => false,
                        'msg'     => __('Please wait 5 minutes to resend an activation code!', 'enginethemes')
                    );
                }
            }
