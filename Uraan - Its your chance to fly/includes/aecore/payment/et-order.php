...
abstract class ET_Order
{
    ...
    function __construct($order, $ship = array()) {
        ...
        if ( ! is_array( $order ) ) {
            ...
            if ($order_id > 0) {
                ...
                $this->_vat_fee = get_post_meta($this->_ID, 'et_order_vat_fee', true);
                ...
                if( empty($this->_order_type) ){
                    if( isset($order->post_type ) ){
                        $this->_order_type = $order->post_type;
                    }
                    else {
                        $this->_order_type = 'order';
                    }
                }
                return $order;
            }
    ...
    */
    function update_order() {
        ...
        $postarr['post_type'] = $this->_order_type;
        ...
        update_post_meta($this->_ID, 'et_order_created_time', strtotime($this->_created_date));
        update_post_meta($this->_ID, 'et_order_paid_date', $this->_paid_date);
        if($this->_product_type == 'mjob_order')
            update_post_meta($this->_ID, 'et_order_fee_buyer', ae_get_option('order_commission_buyer', 0));
    ...
    */
    ...
    protected function calculate_vat_fee( $total ) {
        return apply_filters('mje_apply_vat_fee', $total);
    }
    ...
    public function set_product($product = array()) {
        ...
        foreach ($product as $key => $value) {
            ...
            $p = array(
                'ID' => $key,
                'NAME' => $value['title'],
                'AMT' => $this->calculate_vat_fee($value['price']) ,
                'QTY' => $value['qty'],
        ...
        $this->_total = $this->calculate_vat_fee($total);
    ...
    public function add_product($product, $number = 1) {
        ...
        $this->_products[$product['ID']] = array(
            'ID' => $product['ID'],
            'NAME' => $product['title'],
            'AMT' => $this->calculate_vat_fee($product['price']),
            'QTY' => $number,
