...
class ET_InvalidVisitor extends ET_PaymentVisitor {
	...
	function setup_checkout (ET_Order $order) {
		return array('ACK' => false, 'msg' => __("Invalid payment gateway",'enginethemes'));
	}
...
class ET_PaypalVisitor extends ET_PaymentVisitor
{
	...
	function setup_checkout ( ET_Order $order ) {
		...
		$url		= 	$settings['return'];
		$cancel_url	=	et_get_page_link ('cancel-payment');
	...
	function do_checkout ( ET_Order $order ) {
		...
		ae_paypal_log("Process payment when post back - order status: " . $order_datas['status']);
    ...
    function do_checkout_get_back ( ET_Order $order ) {
	    ae_paypal_log("IPN post back: ", $_POST);
         ...
         * check PDT
         */
	    ae_paypal_log("Request data in checkout get back: ", $_REQUEST);
        ...
        if( isset($_POST['payment_status'])  && ($_POST['payment_status'] == 'Completed' || $_POST['payment_status'] == 'Pending')) {
            ...
            if( $mc_gross == $order['total'] && $mc_currency == $order['currencyCodeType']
                ...
                return array (
                    'ACK'		=> false,
                    'payment'	=>	'simplePaypal',
                    'msg'		=> __("Fraudulent", 'enginethemes'),
                );
            }
        }
        ...
        return array(
    		'ACK' 		=> false,
            'payment'	=>	'simplePaypal',
    		'msg' 		=> __('Payment False', 'enginethemes')
    	);
    }
}
	...
	function do_checkout ( ET_Order $order ) {
		if(!isset($_REQUEST['order_number'])) {
			return array (
				...
				'payment'	=> 	'2checkout',
				'reponse'	=>	array (
					'S_MESSAGE'		=>	__("MD5 False",'enginethemes'),
					'L_MESSAAGE' 	=>  __("Md5 check sum not match!",'enginethemes')
				),
		...
		if( $md5_key == $_REQUEST['key'] ) {
			...
			if( $payment->get_mode () == 'Y' ) {
				...
				return array (
					...
					'reponse'	=>	array (
						'S_MESSAGE'		=>	__("MD5 False",'enginethemes'),
						'L_MESSAAGE' 	=>  __("In  Demo mode the 2Checkout order number is changed to '1' which will cause the hash to fail this. This is intentional so that merchants selling downloadable products and services can compare the hash to determine whether to provide a product to a customer",'enginethemes')
					),
	...
	function setup_checkout (ET_Order $order ) {
		...
		if(isset($order['total_before_discount']) && $order['total_before_discount'] > $order['total'] ) {
			...
			$pro[]	=	array(
					...
					'L_DESC'	=> __("You have use a coupon code to get discount", 'enginethemes'),
		...
		if( isset($_REQUEST['token'])) {
			if($payment->get_digital_key($order['ID']) == $_REQUEST['token'] ) {
				...
				return array (
					'ACK'		=> 	false,
					'payment'	=>	 'google_checkout',
					'S_MESSAGE'	=>	__("Fraudulent or error!",'enginethemes'),
					'payment_status'	=>  'error'
				);
			}
		}
