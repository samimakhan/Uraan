...
class AE_Category extends AE_Term {
	...
	function change_color(){
		if ( !empty($_REQUEST['content']['term_id']) && !empty($_REQUEST['content']['color']) ){
			...
			$resp = array(
				'success'   => true,
				'msg'       => sprintf(__('%s color has been updated', 'enginethemes'), $this->_tax_label )
				);
		}
		else {
			$resp = array(
				'success'   => false,
				'msg'       => __("An error has occurred!", 'enginethemes')
				);
		}
		return $resp;
	}
	...
	function change_icon(){
		if ( !empty($_REQUEST['content']['term_id']) && !empty($_REQUEST['content']['icon']) ){
			...
			$resp = array(
				'success'   => true,
				'msg'       => sprintf(__('%s icon has been updated', 'enginethemes'), $this->_tax_label )
				);
		}
		else {
			$resp = array(
...
class AE_BackendCategory extends AE_Category{
			...
			<li class="tax-item color-0">
				<div class="container">
					...
					<div class="input-form input-form-1 color-default">
						...
						<?php } ?>
						<input style="color:#000000;" class="bg-grey-input tax-name <?php echo $class_no_icon.' '.$class_no_color ?>" name="name" data-tax="<?php echo $this->taxonomy ?>" autocomplete="off" placeholder="<?php _e('Add a category', 'enginethemes') ?>" type="text" />
					</div>
				</div>
			</li>
		</ul>
		...
		foreach ($positions as $job_pos) {
			...
			<li class="tax-item" data-id="<?php echo $job_pos->term_id ?>" data-tax="<?php echo $this->taxonomy ?>" data-color="<?php echo isset($colors[$job_pos->term_id]) ? $colors[$job_pos->term_id] : '#000' ?>" data-icon="fa-map-marker" id="tax_<?php echo $job_pos->term_id ?>">
				<div class="container">
					...
					<div class="controls controls-2">
						<a class="button act-open-form" rel="<?php echo $job_pos->term_id ?>"  title="<?php _e('Add sub tax for this tax', 'enginethemes') ?>">
	...
	?>
		<script type="text/template" id="temp_<?php echo $this->taxonomy ?>_delete_confirm">
			<div class="moved-tax">
				<span><?php _e('Move jobs to', 'enginethemes') ?></span>
				...
				<button class="backend-button accept-btn"><?php _e("Accept", 'enginethemes'); ?></button>
...
class AE_CategoryAjax extends AE_Base{
	...
	function sync_term () {
		try {
			...
			if ( empty($_REQUEST['method']) ) throw new Exception(__('There is an error occurred', 'enginethemes'), 400);
			...
			switch ($method) {
				case 'delete':
					...
					if ( !empty($children) )
						throw new Exception(__('You cannot delete a parent category. You need to delete its sub-categories first.', 'enginethemes'));
					...
					if ( is_wp_error( $result ) ){
						...
						throw new Exception(__("Can't delete category", 'enginethemes'));
					}
					break;
				...
				case 'create' :
					...
					if ( empty($term) ) throw new Exception( __('Category name is required', 'enginethemes') );
				...
				case 'update' :
					...
					if ( empty($term) ) throw new Exception( __("Cannot find category", 'enginethemes') );
				...
				default:
					throw new Exception(__('There is an error occurred', 'enginethemes'), 400);
					break;
			}
