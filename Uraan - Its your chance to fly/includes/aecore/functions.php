    ...
    if ( $in_same_term || ! empty( $excluded_terms ) ) {
        ...
        if ( ! empty( $excluded_terms ) && ! is_array( $excluded_terms ) ) {
            ...
            if ( false !== strpos( $excluded_terms, ' and ' ) ) {
                _deprecated_argument( __FUNCTION__, '3.3', sprintf( __( 'Use commas instead of %s to separate excluded terms.', 'enginethemes' ), "'and'" ) );
...
function et_handle_upload_prefilter($file) {
    ...
    if ($space_left < $file_size) $file['error'] = sprintf(__('Not enough space to upload. %1$s KB needed.', 'enginethemes') , number_format(($file_size - $space_left) / 1024));
    if ($file_size > (1024 * get_site_option('fileupload_maxk', 1500))) $file['error'] = sprintf(__('This file is too big. Files must be less than %1$s KB in size.', 'enginethemes') , get_site_option('fileupload_maxk', 1500));
    if (function_exists('upload_is_user_over_quota') && upload_is_user_over_quota(false)) {
        $file['error'] = __('You have used your space quota. Please delete files before uploading.', 'enginethemes');
    }
...
function ae_the_time($from) {
    ...
    if (time() - $from > (7 * 24 * 60 * 60)) {
        return sprintf(__('on %s', 'enginethemes') , date_i18n(get_option('date_format') , $from, true));
    } else {
        return ae_human_time_diff($from) . ' ' . __('ago', 'enginethemes');
    }
}
...
function ae_human_time_diff($from, $to = '') {
    ...
    if ($diff < HOUR_IN_SECONDS) {
        ...
        $since = sprintf(et_number_based(__('%s min', 'enginethemes') , __('%s min', 'enginethemes') , __('%s mins', 'enginethemes') , $mins) , $mins);
    } elseif ($diff < DAY_IN_SECONDS && $diff >= HOUR_IN_SECONDS) {
        $hours = round($diff / HOUR_IN_SECONDS);
        if ($hours <= 1) $hours = 1;
        $since = sprintf(et_number_based(__('%s hour', 'enginethemes') , __('%s hour', 'enginethemes') , __('%s hours', 'enginethemes') , $hours) , $hours);
    } elseif ($diff < WEEK_IN_SECONDS && $diff >= DAY_IN_SECONDS) {
        $hours = round($diff / HOUR_IN_SECONDS);
        $days = round($diff / DAY_IN_SECONDS);
        if ($days <= 1) $days = 1;
        $since = sprintf(et_number_based(__('%s day', 'enginethemes') , __('%s day', 'enginethemes') , __('%s days', 'enginethemes') , $days) , $days);
    } elseif ($diff < 30 * DAY_IN_SECONDS && $diff >= WEEK_IN_SECONDS) {
        $hours = round($diff / HOUR_IN_SECONDS);
        $weeks = round($diff / WEEK_IN_SECONDS);
        if ($weeks <= 1) $weeks = 1;
        $since = sprintf(et_number_based(__('%s week', 'enginethemes') , __('%s week', 'enginethemes') , __('%s weeks', 'enginethemes') , $weeks) , $weeks);
    } elseif ($diff < YEAR_IN_SECONDS && $diff >= 30 * DAY_IN_SECONDS) {
        $hours = round($diff / HOUR_IN_SECONDS);
        $months = round($diff / (30 * DAY_IN_SECONDS));
        if ($months <= 1) $months = 1;
        $since = sprintf(et_number_based(__('%s month', 'enginethemes') , __('%s month', 'enginethemes') , __('%s months', 'enginethemes') , $months) , $months);
    } elseif ($diff >= YEAR_IN_SECONDS) {
        $hours = round($diff / HOUR_IN_SECONDS);
        $years = round($diff / YEAR_IN_SECONDS);
        if ($years <= 1) $years = 1;
        $since = sprintf(et_number_based(__('%s year', 'enginethemes') , __('%s year', 'enginethemes') , __('%s years', 'enginethemes') , $years) , $years);
    }
...
if( !function_exists( 'et_get_page_link' ) ){
    function et_get_page_link($pages, $params = array() , $create = true) {
        ...
        $page_args = array(
            'post_title' => '',
            'post_content' => __('Please fill out the form below ', 'enginethemes') ,
            'post_type' => 'page',
            'post_status' => 'publish'
        );
...
function et_the_time($from) {
    ...
    //
    if (time() - $from > (7 * 24 * 60 * 60)) {
        return sprintf(__('%s', 'enginethemes') , date_i18n(get_option('date_format') , $from, true));
    } else {
        return et_human_time_diff($from) . ' ' . __('ago', 'enginethemes');
    }
}
...
if( !function_exists( 'et_get_ban_expired_period' )) {
    function et_get_ban_expired_period(){
        $expired = array(
            array(
                'label'     => __('1 Day', 'enginethemes'),
                'value'     => '+1 day'
            ),
            array(
                'label'     => __('3 Days', 'enginethemes'),
                'value'     => '+3 days'
            ),
            array(
                'label'     => __('7 Days', 'enginethemes'),
                'value'     => '+7 days'
            ),
            array(
                'label'     => __('15 Days', 'enginethemes'),
                'value'     => '+15 days'
            ),
            array(
                'label'     => __('1 Month', 'enginethemes'),
                'value'     => '+1 month'
            ),
            array(
                'label'     => __('3 Months', 'enginethemes'),
                'value'     => '+3 months'
            ),
            array(
                'label'     => __('6 Months', 'enginethemes'),
                'value'     => '+6 months'
            ),
            array(
                'label'     => __('1 Year', 'enginethemes'),
                'value'     => '+1 year'
            ),
            array(
                'label'     => __('Forever', 'enginethemes'),
                'value'     => '+999 year'
            )
        );
