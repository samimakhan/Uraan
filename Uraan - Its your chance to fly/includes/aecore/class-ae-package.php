...
class AE_Package extends AE_Pack
{
    ...
    function __construct($post_type = 'pack', $meta = array()) {
        ...
        $this->localize = array(
            'backend_text' => array(
                'text' => __('%s for %d days', 'enginethemes') ,
                'data' => array(
                    'et_price',
                    'et_number_posts'
                )
    ...
    function convert($post, $thumbnail = '', $excerpt = false, $singular = false) {
        ...
        if ($result) {
            ...
            if ($align) {
                $result->backend_text = sprintf(__("(%s)%s for %d days", 'enginethemes') , $currency, $result->et_price, $result->et_duration);
            } else {
                $result->backend_text = sprintf(__("%s(%s) for %d days", 'enginethemes') , $result->et_price, $currency, $result->et_duration);
            }
        }
        return $result;
    }
    ...
    public static function limit_free_plan($package) {
        ...
        if ( $package && $package->et_price == 0    ) {
            ...
            if ($number > $package->et_number_posts) {

                $response['success'] = true;
                $response['msg'] = __("You have reached the maximum number of Free posts. Please select another plan.", 'enginethemes');
