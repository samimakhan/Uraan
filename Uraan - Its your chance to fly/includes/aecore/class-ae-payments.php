...
abstract class AE_Payment extends AE_Base
{
    ...
    function process_payment($payment_return, $data) {
        ...
        et_track_payment('hook:ae_process_payment_action method:process_payment');
        ...
        if (!isset($data['ad_id']) || !$data['ad_id']){
            et_track_payment('empty_adID::exit');
        ...
        if (!$payment_return['ACK']) {
            et_track_payment('ack_field::exit');
            return 0;
        }
        ...
        et_track_payment('process_payment line 128 - payment_type: '.$payment_type);
        ...
        if( is_super_admin() ){
            et_track_payment('supper_admin and auto set publish post. post_id: '.$ad_id);
        ...
        if ('PENDING' != strtoupper($payment_return['payment_status'])) {
            et_track_payment('payment successful');
            et_track_payment('set post_status via payment_status. '.$payment_return['payment_status']);
            wp_update_post($post_data);

            // paid
            update_post_meta($ad_id, 'et_paid', 1);
        } else {
            et_track_payment('payment fail/pending/canot_heck');
    ...
    function setup_orderdata($data) {
        ...
        if ($adID) {
            ...
            if ($author != $job->post_author && !current_user_can('manage_options')) {
                $author_error = __("Post author information is incorrect!", 'enginethemes');
                $errors[] = $author_error;
            }
        }
    ...
    function setup_payment() {
        ...
        if (empty($plans)) {
            wp_send_json(array(
                'success' => false,
                'msg' => __("There is no payment plan.", 'enginethemes')
            ));
        }
        ...
        if ($nvp['ACK']) {
            ...
            $response = array(
                'success' => false,
                'paymentType' => $paymentType,
                'msg' => __("Invalid payment gateway", 'enginethemes')
            );
        }
        /**
    ...
    public function member_payment_process($payment_return, $data) {
         ...
         */
        do_action('ae_member_process_order', $order_pay['payer'], $order_pay, $data);
        return true;
    }
