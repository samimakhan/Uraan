        ...
        if(!is_admin() && $query->is_main_query()) {
            if($query->is_search()) {
                ...
                if(isset($_GET['language_ids']) && !empty($_GET['language_ids'])) {
                    ...
                    $sql = "SELECT DISTINCT post_author FROM `wp_posts` as wp JOIN wp_term_relationships AS wtr ON wp.ID = wtr.object_id WHERE post_type = 'mjob_profile' AND wtr.term_taxonomy_id IN ($language)";
