...
if( $order_id && ($user_ID == $order_data['payer'] || is_super_admin($user_ID)) ) {
    ...
    if($order_id ) {
        ...
        $is_order_processed = get_post_meta($order_id, 'et_order_is_process_payment');

        if( !$is_order_processed ){
            $process_type = isset($session['process_type']) ? $session['process_type'] : '';
            et_track_payment('access order: '.$order_id . ' process_type: '.$process_type);
            et_log('access order: '.$order_id . ' process_type: '.$process_type);

            if( $process_type == 'buy' ){
                et_track_payment('condition first: process_payment');
                $payment_return = MJE_Checkout::process_payment( $payment_type, $session );
            } else {
                et_track_payment('condition second: ae_process_payment -payment_type : '.$payment_type);
                $payment_return = ae_process_payment($payment_type, $session);
            }
            update_post_meta($order_id, 'et_order_is_process_payment', true);
            et_destroy_session();
        } else {
            et_track_payment('Order ID '.$order_id.' is processed.');
        }
    } else{
        et_track_payment('Empty order');
        et_log('Empty order');
